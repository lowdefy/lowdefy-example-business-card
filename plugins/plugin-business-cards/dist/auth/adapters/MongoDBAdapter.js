...
/** Converts from string to ObjectId */ function _id(hex) {
    if (hex?.length !== 24) return new ObjectId();
    return new ObjectId(hex);
}
function MongoDBAdapter({ properties  }) {
    console.log('MongoDBAdapter', {
        properties
    });
    ...
    return {
        ...
        async updateUser (data) {
            ...
            const { _id: _id1 , ...user } = to(data);
            const result = await (await db).users.findOneAndUpdate({
                _id: _id1
            }, {
                $set: user
            }, {
        ...
        async deleteUser (id) {
            ...
            const m = await db;
            await Promise.all([
                m.accounts.deleteMany({
                    userId
                }),
                m.sessions.deleteMany({
                    userId: userId
                }),
                m.users.deleteOne({
                    _id: userId
        ...
        async unlinkAccount (provider_providerAccountId) {
            ...
            const { value: account  } = await (await db).accounts.findOneAndDelete(provider_providerAccountId);
            return from(account);
        },
        async getSessionAndUser (sessionToken) {
            console.log('getSessionAndUser', sessionToken);
            const session = await (await db).sessions.findOne({
                sessionToken
            });
        ...
        async updateSession (data) {
            ...
            const result = await (await db).sessions.findOneAndUpdate({
        ...
        async deleteSession (sessionToken) {
            ...
            const { value: session  } = await (await db).sessions.findOneAndDelete({
                sessionToken
            });
            return from(session);
        },
        ...
        async useVerificationToken (identifier_token) {
            const { value: verificationToken  } = await (await db).verificationTokens.findOneAndDelete(identifier_token);
